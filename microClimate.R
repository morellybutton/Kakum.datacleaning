#calculating daily VPD and other metrics for each plot from Campbell Sci and USB dataloggers rawdata
...
#plotnums=c("KW500M3","KW100M1","KW100M3","KW1KM3","KWFOREST","AH500M2","AH500M3","AH100M3","AH5KM2","AHFOREST")
...
#AH equation = 2.16679*SVP*RH/T(in K) [in g/m3]
...
for (i in 1:length(nums)){
  dat<-data.frame(read.csv(paste0(getwd(),"/",root,nums[i],"_Table1.csv"), stringsAsFactors=FALSE))
  ...
  dat[is.na(as.numeric(dat[,12])),12]=0
  ...
  #find plot number for metstation
...
#met data
met<-data.frame(read.csv(paste0(getwd(),"/",root,"11_Table1.csv"), stringsAsFactors=FALSE))
#calculate VPD
...
final<-do.call(rbind.data.frame,d)
write.csv(final,paste0(getwd(),"/",root,"11_summary.csv"))

############USB dataloggers
root="NCRC_"
#find list of files
names<-list.files(path=paste0(getwd(),"/dataloggers"),pattern=root,all.files = FALSE,full.names = TRUE)
...
combo<-as.data.frame(matrix(nrow=length(name[,1]),ncol=6),stringsAsFactors =FALSE)
for(i in 1:length(name[,1])){
  p<-grep(as.character(name[i,2]),ns$name)
  combo[i,1:2]<-cbind(as.character(name[i,2]),as.character(name[i,3]))
  combo[i,3]<-cbind(as.character(ns[p,5]))
  combo[i,4:6]<-cbind(ns[p,2],ns[p,3],ns[p,6])
}
rm(p)
...
for (i in 1:length(names)){
  ...
  p<-str_split(str_split(names[i],root)[[1]][2],".csv")
  p<-paste0(root,p[[1]][p[[1]]!=""])
  n<-as.Date(name[grep(p,as.character(name[,3])),1],format="%d/%m/%y")+1
  if(p=="NCRC_1") n=n[1]
  ...
  final<-do.call(rbind.data.frame,d)
  #find plot number for metstation
  g<-strsplit(names[i],root)
  g2<-strsplit(g[[1]][2],".csv")
  #as.character(name[name[,3]==paste0(root,g2[[1]][1]),2])
  write.csv(final,paste0(getwd(),"/dataloggers/summary/",root,g2[[1]][1],"_summary.csv"))
