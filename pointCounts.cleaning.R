...
for (i in 1:length(snames)){
  ...
  dF<-read.xls(paste0(getwd(),"/Biodiversity/PointCounts_rawdata_",year,".xlsx"),sheet=snames[i])
  #remove factors
...
write.csv(final.1,paste0(getwd(),"/Biodiversity/Pointcount_spplist ",year,".csv"))

comp<-read.csv(paste0(getwd(),"/Biodiversity/Pointcount_spplist ",year,".csv"))
...
for (i in 1:length(snames)){
  ...
  dF<-dF[,1:7]
  write.csv(dF,paste0(getwd(),"/Biodiversity/Pointcount_rawdata_",snames[i],".",gsub(" ",".",year),".csv"))
 }
...
for (i in 1:length(snames)){
  #data<-read.csv(paste0(getwd(),"/",site,"/Biodiversity/PointCounts_rawdata_",snames[i],".csv"))
  dF<-read.csv(paste0(getwd(),"/Biodiversity/Pointcount_rawdata_",snames[i],".",gsub(" ",".",year),".csv"))
  #remove factors
  dF<-data.frame(lapply(dF, as.character), stringsAsFactors=FALSE)
  #extract all common names
